$(function (){
	...
	$.get('./data/type.json', function (data){
		...
		for(var i = 0; i < data.length; i++){
			html += '<div style="border-radius:'+w/2+'px;">'+data[i].Name+'</div>';
		}
		$('#typeList').html(html);
	});
	// 为电影类型添加点击事件
	...
	$('#movieList').on('click', 'div', function (e){
		...
		$.ajax({
			...
			success:function (data){
				...
				html += '<dt>原名 :</dt><dd>&nbsp;&nbsp;'+data.current_title+'</dd>';
				html += '<dt>评分 :</dt><dd>&nbsp;&nbsp;'+data.rating.average+'</dd>';
				html += '<dt>导演 :</dt><dd>&nbsp;&nbsp;'+data.directors[0].name+'</dd>';
				html += '<dt>主演 :</dt><dd>&nbsp;&nbsp;'+data.casts[0].name+'</dd>';
